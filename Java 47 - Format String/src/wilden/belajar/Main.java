/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package wilden.belajar;

import java.util.Formatter;

/**
 *
 * @author wilden
 */
public class Main {

    /**
     * @param args the command line arguments
     */
    public static void main(String[] args) {
        // TODO code application logic here
        
        String nama = "Udin";
        String namaLain = "Ujang";
        int umur = 17;
        
        // output : Nama saya adalah Udin, umur saya adalah 17
        System.out.println("Nama saya adalah " + nama + ", umur saya adalah " + umur);
        System.out.println();
        
        // Cara lainnya adalah dengan format string
        System.out.printf("Nama saya adalah %s, umur saya adalah %d", nama, umur);
        System.out.println();
        
        // Conversion : f = floating point, d = integer, c = character,
        //              s = string, b = boolean
        
        // Struktur Format = %[argumen_index$][flags][width][.precision]conversion
        
        // [argumen_index$]
        System.out.println("\n===== [argumen_index$] =====");
        
        // udin, wahai udin, kemana saja kamu ujang
        System.out.printf("%1$s, wahai %1$s, kemana saja kamu %2$s \n", nama, namaLain);
        
        // umur udin berapa?, udin menjawab: 17, wah masih muda ya umurnya 17
        System.out.printf("\numur %1$s berapa? \n%1$s menjawab: %2$d \nWah masih muda ya umurnya %2$d\n",nama,umur);
        
        // [flags]
        System.out.println("\n===== [flags] =====");
        int int1 = 5;
        int int2 = 8;
        int hasil = int1 - int2;
        System.out.printf("%d - %d = %+d \n", int1, int2, hasil);
        
        // [width]
        System.out.println("\n===== [width] =====");
        int int3 = 1000;
        System.out.println("INTEGER");
        System.out.printf("d        %d \n", int3);
        System.out.printf("5d       %5d \n", int3);
        System.out.printf("-5d      %-5d \n", int3);    // flags = "-" artinya rata kiri
        System.out.printf("+5d      %+5d \n", int3);    // flag akan mengambil slot di dalam format
        System.out.printf("+-6d     %+-6d \n", int3);   // flag bisa digabungkan
        System.out.printf("10d      %10d \n", int3);
        System.out.printf(" 10d     % 10d \n", int3);
        System.out.printf("010d     %010d \n", int3);   // flag = "0" kita tambahkan leading "0" didepan
        System.out.printf("+010d    %+010d \n", int3);
        
        int int4 = 1000000000;
        System.out.printf("-,15d    %-,15d \n", int4);  // flags = "," tanda untuk delimeter perseribu
        
        
        // FLOATING POINT
        System.out.println("\n===== FLOATING POINT =====");
        float float1 = 1.543f;
        System.out.printf("f    %f \n", float1);
        System.out.printf("+9f    %+9f \n", float1);    // floating point untuk mengambil width dengan 6 decimal
        
        
        // [.precision]
        System.out.println("\n===== [.precision] =====");
        float float2 = 15.678f;
        System.out.printf("f    %f \n", float2);
        System.out.printf(".1f    %f \n", float2);
        System.out.printf(".2f    %f \n", float2);
        System.out.printf("8.2f    %f \n", float2);     // gabungkan dengan width
        System.out.printf("+08.2f    %f \n", float2);   // gabungkan dengan width dan flag
        
        // CONTOH
        System.out.println("\n===== CONTOH =====");
        String nama2 = "Ucup";
        float IPK = 3.785123567653f;
        
            // struktur format = %[argumen_index$][flags][width][.precision]conversion
        System.out.printf("\nIPK %1$s berapa? \n%1$s: Saya dapet %2$+5.2f\n", nama2, IPK);
        
        
        // KESIMPULAN
        System.out.println("\n===== KESIMPULAN =====");
        String info_biasa = "Nama: " + nama2 + ", IPK = " + IPK;
        System.out.println("Biasa -> " + info_biasa);
        
        String info_format = String.format("Nama: %s, IPK = %2$+5.2f", nama, IPK);
        System.out.println("String Format -> " + info_format);
        
        // Save Format ini kedalam string builder
        StringBuilder builder_info = new StringBuilder();
        Formatter formatBuilder = new Formatter(builder_info);
        
        formatBuilder.format("Nama: %s, IPK = %2$+5.2f", nama2, IPK);
        System.out.println("String builder format -> " + builder_info);
    }
    
    
}
